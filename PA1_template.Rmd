...
```{r,fig.height=4,fig.path="figures/"}
...
The **mean** steps per day is `r mean.steps`.  The **median** steps per day is `r median.steps`.
...
```
On average, the **max steps occur at interval `r steps.interval.max`**, or, in other words, 5-min. interval number `r steps.intervalNum.max` of the day.
...
For the imputed data, the mean steps per day is `r imputed_mean.steps`.  The median steps per day is `r imputed_median.steps`.
